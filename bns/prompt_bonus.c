...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/15 16:13:05 by bakgun            #+#    #+#             */
...
static int	check_input(t_vars *vars, int condition, int in_quote)
{
	char	quotes;
	int		i;
	...
	if ((!condition && destroy_para(vars, 0)) || !seek_operator(vars))
		return (0);
	return (3);
}
...
int	handle_prompt(t_vars *vars, int condition)
{
	...
	if (!vars->input)
		return (2);
	if (check_input(vars, condition, 0) != 3)
		return (1);
